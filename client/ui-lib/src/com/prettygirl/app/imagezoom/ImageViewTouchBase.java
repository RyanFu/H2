...
public class ImageViewTouchBase extends ImageView implements IDisposable {
    ...
    protected void postScaleWithRadius(float scale, float radius, float centerX, float centerY) {
        //        if (radius != 0) {
        //            mSuppMatrix.postRotate(radius, centerX, centerY);
        //        } else {
        mSuppMatrix.postScale(scale, scale, centerX, centerY);
        //        }
        setImageMatrix(getImageViewMatrix());
    ...
    public void rotateBy(float degrees) {
        float cx = getWidth() / 2F;
        float cy = getHeight() / 2F;
        mSuppMatrix.postRotate(degrees, cx, cy);
        setImageMatrix(getImageViewMatrix());
    }
    ...
    public void rotateTo(float degrees) {
        float cx = getWidth() / 2F;
        float cy = getHeight() / 2F;
        mSuppMatrix.setRotate(degrees, cx, cy);
        setImageMatrix(getImageViewMatrix());
    }

    public void zoomBy(float scaleBy, float durationMs) {
        float cx = getWidth() / 2F;
        float cy = getHeight() / 2F;
        zoomTo(scaleBy * getScale(), cx, cy, durationMs);
    }
