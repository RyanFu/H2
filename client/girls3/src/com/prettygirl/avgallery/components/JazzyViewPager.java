...
public class JazzyViewPager extends ViewPager {
    ...
    public void setPageMargins(int margin) {
        mPageMargin = margin;
    }
        ...
        if (mState != State.IDLE) {
            ...
            if (right != null) {
                ...
                if (mRot < -90.0f) {
                    ...
                    mTrans = -getWidth() - mPageMargin + positionOffsetPixels;
                    ViewHelper.setPivotX(right, right.getMeasuredWidth() * 0.5f);
                    ViewHelper.setPivotY(right, right.getMeasuredHeight() * 0.5f);
                    ViewHelper.setTranslationX(right, mTrans);
                    ViewHelper.setRotationY(right, mRot);
    ...
    public void startAutoPlay(int interval) {
        mAutoPlayRunnable = new AutoPlayRunnable(interval);
        postDelayed(mAutoPlayRunnable, 800);
    }
    ...
    private class AutoPlayRunnable implements Runnable {
        ...
        AutoPlayRunnable(int interval) {
            this.interval = interval;
        }

        @Override
        public void run() {
            if (getCurrentItem() < getAdapter().getCount() - 1 && ((Activity) getContext()).isFinishing() == false) {
